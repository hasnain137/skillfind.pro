        ...
        return (
            <div className="min-h-screen flex items-center justify-center bg-[#FAFAFA]">
                <Card padding="lg" className="max-w-md text-center">
                    ...
                    <h1 className="text-xl font-bold text-[#333333] mb-2">{t('unavailable.title')}</h1>
                    <p className="text-sm text-[#7C7373]">
                        {t('unavailable.description')}
                    </p>
                    <Link href="/search">
                        <Button className="mt-4">{t('unavailable.cta')}</Button>
                    </Link>
                </Card>
            </div>
        );
                    ...
                    <div className="flex flex-col md:flex-row gap-6 items-start">
                            ...
                            <div className="flex flex-wrap items-center gap-4 text-sm">
                                ...
                                {professional.averageRating > 0 && (
                                    <div className="flex items-center gap-2 bg-white/20 px-3 py-1.5 rounded-full">
                                        ...
                                        <span className="text-blue-100">({t('hero.reviews', { count: professional.totalReviews })})</span>
                                    </div>
                                )}
                        ...
                        <div className="flex gap-4 md:flex-col">
                            <div className="text-center bg-white/20 p-4 rounded-xl min-w-[100px]">
                                ...
                                <p className="text-xs text-blue-100">{t('hero.jobsDone')}</p>
                            </div>
                            {professional.yearsOfExperience && (
                                <div className="text-center bg-white/20 p-4 rounded-xl min-w-[100px]">
                                    <p className="text-3xl font-bold">{professional.yearsOfExperience}</p>
                                    <p className="text-xs text-blue-100">{t('hero.yearsExp')}</p>
                                </div>
                            )}
                        </div>
                    </div>
                ...
                <div className="grid gap-6 lg:grid-cols-3">
                    ...
                    <div className="lg:col-span-2 space-y-6">
                                            ...
                                            <div className="flex items-start justify-between gap-4">
                                                ...
                                                {(service.priceFrom || service.priceTo) && (
                                                    <div className="text-right">
                                                        ...
                                                        <p className="text-xs text-[#7C7373]">{t('services.perProject')}</p>
                                                    </div>
                                                )}
                                            </div>
                                        </div>
                        ...
                        {reviews.length > 0 && (
                            <Card padding="lg">
                                <h2 className="text-lg font-bold text-[#333333] mb-4 flex items-center gap-2">
                                    <span>⭐</span> {t('sections.reviews', { count: professional.totalReviews })}
                                </h2>
                                        ...
                                        <div key={review.id} className="border-b border-[#E5E7EB] pb-4 last:border-0">
                                            ...
                                            {review.wouldRecommend && (
                                                <div className="flex items-center gap-1 text-xs text-green-600">
                                                    <span>✓</span>
                                                    <span>{t('reviews.wouldRecommend')}</span>
                                                </div>
                                            )}
                    ...
                    <div className="space-y-6">
                        ...
                        <Card padding="lg" className="bg-gradient-to-br from-blue-50 to-white border-blue-200">
                            <h3 className="text-base font-bold text-[#333333] mb-4">{t('sections.quickStats')}</h3>
                            <div className="space-y-3">
                                <div className="flex items-center justify-between py-2 border-b border-[#E5E7EB]">
                                    <span className="text-sm text-[#7C7373]">{t('stats.completion')}</span>
                                    <span className="text-sm font-bold text-blue-600">{professional.profileCompletion}%</span>
                                </div>
                                <div className="flex items-center justify-between py-2 border-b border-[#E5E7EB]">
                                    <span className="text-sm text-[#7C7373]">{t('stats.completedJobs')}</span>
                                    <span className="text-sm font-bold text-[#333333]">{professional.completedJobs}</span>
                                </div>
                                <div className="flex items-center justify-between py-2 border-b border-[#E5E7EB]">
                                    <span className="text-sm text-[#7C7373]">{t('stats.avgRating')}</span>
                                    <span className="text-sm font-bold text-[#333333]">
                                        {professional.averageRating > 0 ? `${professional.averageRating.toFixed(1)} ⭐` : t('stats.noRatings')}
                                    </span>
                                </div>
                                <div className="flex items-center justify-between py-2">
                                    <span className="text-sm text-[#7C7373]">{t('stats.totalReviews')}</span>
                        ...
                        {!currentUserClient && (
                            <Card padding="lg" className="bg-gradient-to-br from-green-50 to-white border-green-200 text-center">
                                ...
                                <h3 className="text-base font-bold text-[#333333] mb-2">
                                    {t('cta.readyToHire', { name: professional.user.firstName || '' })}
                                </h3>
                                <p className="text-sm text-[#7C7373] mb-4">
                                    {t('cta.signInDesc')}
                                </p>
                                <Link href="/signup">
                                    <Button className="w-full mb-2">{t('cta.getStarted')}</Button>
                                </Link>
                                <Link href="/login">
                                ...
                                {professional.remoteAvailability && (
                                    <div className="mt-3 pt-3 border-t border-[#E5E7EB]">
                                        <div className="flex items-center gap-2 text-sm text-green-600">
                                            <span>✓</span>
                                            <span>{t('location.remoteAvailable')}</span>
                                        </div>
                                    </div>
                                )}
                            </Card>
                        ...
                        {professional.yearsOfExperience && (
                            <Card padding="lg">
                                <h3 className="text-base font-bold text-[#333333] mb-3 flex items-center gap-2">
                                    <span>🎓</span> {t('sections.experience')}
                                </h3>
                                <p className="text-2xl font-bold text-blue-600 mb-1">
                                    {t('experience.years', { count: professional.yearsOfExperience })}
                                </p>
                                <p className="text-xs text-[#7C7373]">
                                    {t('experience.desc')}
                                </p>
